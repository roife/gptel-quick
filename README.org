...
- or ~q~/~C-g~ (or =keyboard-quit=) to clear it.
...
Posframe responses are rendered as unfolded Markdown with =markdown-ts-mode= when it is available.  If the [[https://github.com/tumashu/posframe][posframe]] package (optional) is not installed or cannot create a child frame, the response is messaged to the echo area instead.  On Emacs 31 or newer, posframe can use TTY child frames to show the popup in a terminal too.
...
Lisp callers can pass a third argument to =gptel-quick= to override request options for one lookup.  The plist accepts =:system= for a custom system message and =:max-tokens= for a token limit (or nil for no limit).  These options are preserved when ~+~ requests a longer response.
