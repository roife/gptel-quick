...
If the [[https://github.com/tumashu/posframe][posframe]] package (optional) is not installed or cannot create a child frame, the response is messaged to the echo area instead.  On Emacs 31 or newer, posframe can use TTY child frames to show the popup in a terminal too.
